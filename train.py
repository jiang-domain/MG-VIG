...
#源vig代码from timm.data import Dataset, resolve_data_config, Mixup, FastCollateMixup, AugMixDataset #, create_loader
#修改代码如下
from torch.utils.data import Dataset  # 使用 PyTorch 原生 Dataset
...
parser.add_argument('--find-unused-parameters', action='store_true', default=False,
                    help='Enable find_unused_parameters in DistributedDataParallel (for multi-branch/multi-scale models')
#为解决多卡ddp问题，新加代码如上
...
def main():
    ...
    #启用 EMA 后验证集（尤其是 EMA 模型）损失异常增大的核心原因是EMA 模型参数未正确跟踪主模型的有效更新，
    #导致其参数状态异常（如接近随机初始化或未同步关键统计量）
    model_ema = None
    if args.model_ema:
    ...
    if args.distributed:
        ...
        if has_apex and use_amp != 'native':
            ...
            model = NativeDDP(model, device_ids=[args.local_rank], find_unused_parameters=args.find_unused_parameters)  # can use device str in Torch >= 1.1
    ...
    train_dir = '/home/yantianjiang/davig_image1k/data/train'
    if not os.path.exists(train_dir):
        _logger.error('Training folder does not exist at: {}'.format(train_dir))
        exit(1)
    #源码，出错 dataset_train = Dataset(train_dir)
    #替换为如下
    # 用 ImageFolder 实例化，传入数据目录
    dataset_train = ImageFolder(train_dir)
    #上为所替换
    ...
    eval_dir = '/home/yantianjiang/davig_image1k/data/val'
    if not os.path.isdir(eval_dir):
        eval_dir = os.path.join(args.data, 'validation')
        if not os.path.isdir(eval_dir):
            _logger.error('Validation folder does not exist at: {}'.format(eval_dir))
            exit(1)
    #源码dataset_eval = Dataset(eval_dir)
    #如下修改
    dataset_eval = ImageFolder(eval_dir)
    #如上修改
...
#加入新的解决ema模型的 BatchNorm 运行统计量（running mean/var）未做对齐/校准如下

#加入新的解决ema模型的 BatchNorm 运行统计量（running mean/var）未做对齐/校准如上
def train_epoch(
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        if model_ema is not None:
            model_ema.update(model.module if hasattr(model, 'module') else model)
        num_updates += 1
